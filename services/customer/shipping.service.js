...
class ShippingService {
  ...
  static SOUTHERN_PROVINCES = [
    ...
    "Long An", "Bến Tre", "Vĩnh Long", "Trà Vinh", "Hậu Giang", 
    "Tiền Giang", "An Giang", "Kiên Giang", "Đồng Nai", "Đồng Tháp", 
    "Bạc Liêu", "Sóc Trăng", "Cà Mau", "TP Hồ Chí Minh", "Cần Thơ"
  ];
  ...
  static NORTHERN_PROVINCES = [
    ...
    "Quảng Ninh", "Bắc Ninh", "Hà Nam", "Hà Nội", "Hải Dương", 
    "Hưng Yên", "Hải Phòng", "Nam Định", "Ninh Bình", 
    "Thái Bình", "Vĩnh Phúc"
  ];
  ...
  static async getShippingMethods(province) {
    ...
    const availableMethods = methods.map(method => {
      if (method.name === "Hỏa tốc" && !this.SOUTHERN_PROVINCES.includes(province)) {
        return null;
      }
      return {
        ...method.toObject(),
  ...
  static async calculateShippingFee(methodId, province) {
    ...
    if (method.name === "Hỏa tốc" && !this.SOUTHERN_PROVINCES.includes(province)) {
      throw new AppError("Express shipping is only available in Southern provinces", 400);
    }
